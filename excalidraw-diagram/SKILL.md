...
description: Use when the user wants an Excalidraw diagram that explains something visually through architecture maps, workflows, process flows, system relationships, concept diagrams, or argument-driven visuals exported as `.excalidraw` JSON.
metadata:
  tags: "excalidraw, diagram, visualization, architecture, workflow, process-flow, systems-design, concept-map, json-output, visual-explanation, annotated-diagram"
...
```bash
scripts/render <path-to-file.excalidraw> [output.png] [scale] [max_width]
```

Arguments:

- `input.excalidraw` - source Excalidraw file
- `output.png` - optional output path, defaults to the same name with `.png`
- `scale` - optional device pixel ratio, defaults to `2`
- `max_width` - optional max rendered width in CSS pixels, defaults to `1920`
...
**7. Clean up** — After all render/view/fix work is complete, delete the temporary `.png` render file so the workspace does not accumulate generated artifacts.
